...
BLOOP_VERSION="1.4.2"
SBT_VERSION="1.3.13"
SCALAFMT_VERSION="2.6.1"

export PATH=$PATH:/usr/local/openjdk-8/bin

curl -L https://github.com/scalacenter/bloop/releases/download/$BLOOP_VERSION/install.py > ~/bloop_install.py
python ~/bloop_install.py --dest $METALS_DIR/bloop_local
curl -L https://piccolo.link/$SBT_VERSION.tgz > ~/sbt.tar.gz
...
$METALS_DIR/cs fetch org.scalameta:scalafmt-cli_2.12:$SCALAFMT_VERSION --cache=$METALS_DIR/coursier 
